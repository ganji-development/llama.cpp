...
ggml_cgraph * clip_graph_moss_music::build() {
    ...
    //
    // build_vit takes its trailing parameters through build_vit_opts, so this
    // collects through the post-residual hook rather than a layers_out
    // parameter. callback_layer_out fires mid-layer and is the wrong tensor.
    std::vector<ggml_tensor *> layers_out;
    build_vit_opts vit_opts;
    vit_opts.callback_layer_post = [&layers_out](ggml_tensor * out, int /*il*/) {
        layers_out.push_back(out);
    };
    cur = build_vit(
            cur, n_pos,
            NORM_TYPE_NORMAL,
